$(function() {
    ...
    options.on('click', function() {
            var title = $('.subject-title').text();
            var option = $(this);
            if (!$('.answer-box').text()) {
                option.find('label').toggleClass('check');
            }
            if (title == "单选题" || title == "判断题") {
                judgeAnswer(checkRadio);
            }
        })
        //提交答案添加效果 正确为绿色，错误为红色
    $('#btnSubmit').on('click', function() {
            judgeAnswer(checkMultiple);
        })
        //判断题目对错
    function judgeAnswer(fun) {
        //options为全局变量
        var flag = 1;
        var index = $('.item').text() - 1; //题目序号
        console.log(data[index]);
        if (!$(".radio.check").length) {
            alert('喂喂喂，别走神啊！你怕是要交白卷')
        } else fun(flag, index);
        $('.answer-box').text(data[index]['答案']);
    }

    function checkRadio(flag, index) {
        var CheckOption = $(".radio.check")
        if (CheckOption.data('status') == 'false') {
            flag = 0
        }
        AddAnswerStatus(flag, index);
    }

    function checkMultiple(flag, index) {
        for (var i = 0; i < optionLen; i++) {
            var option = options.eq(i).find('label');
            //判断错误的条件,选中不对，对的没选中
            if ((option.data('status') == 'false' && option.hasClass('check')) || (option.data('status') == 'true' && !option.hasClass('check'))) {
                flag = 0;
            }
        }
        AddAnswerStatus(flag, index);
    ...
    //判断题目状态
    function AddAnswerStatus(flag, index) {
        console.log(flag);
        var index = index % 100;
        if (flag == true) {
    ...
    function getSubject(id) {
        if (id >= 0 && id <= subjectLen) {
            ...
            options.each(function() {
                    ...
                    $(this).find('label').removeClass('false check correct');
                })
                //设置题目类型
